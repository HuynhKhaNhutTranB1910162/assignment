...
@section('content')
    <main class="h-full pb-16 overflow-y-auto">
            ...
            <div class="grid gap-6 mb-8 md:grid-cols-2">
                <div class="min-w-0 p-4 text-white bg-purple-600 rounded-lg shadow-xs">
                    <div class="flex justify-center items-center">
                        <img src="{{ asset('storage/' . Auth::guard('admin')->user()->image) }}" alt="Avatar" style="width: 100px;" class="rounded-full overflow-hidden">
                    </div>
                    <br>
                    <ul style="border-top: 1px solid #d7d7d7;border-bottom: 1px solid #d7d7d7;padding: 15px 0;margin-bottom: 26px;">
                        <li style="list-style: none;font-size: 16px;line-height: 40px;overflow: hidden;">
                            Tên người dùng <span style="font-weight: 700;float: right;">
                                {{ Auth::guard('admin')->user()->name }}</span></li>
                        <li style="list-style: none;font-size: 16px;line-height: 40px;overflow: hidden;">
                            Email <span style="font-weight: 700;float: right;">
                                {{ Auth::guard('admin')->user()->email }}</span></li>
                        <li style="list-style: none;font-size: 16px;line-height: 40px;overflow: hidden;">
                            Số điện thoại <span style="font-weight: 700;float: right;">
                                {{ Auth::guard('admin')->user()->phone }}</span></li>
                        @if( (Auth::guard('admin')->user()->is_admin) == 0)
                        <li style="list-style: none;font-size: 16px;line-height: 40px;overflow: hidden;">
                            Chức vụ <span style="font-weight: 700;float: right;">Admin chính</span></li>
                        @endif
                        @if( (Auth::guard('admin')->user()->is_admin) == 1)
                            <li style="list-style: none;font-size: 16px;line-height: 40px;overflow: hidden;">
                                Chức vụ <span style="font-weight: 700;float: right;">Nhân viên nhập kho</span></li>
                        @endif
                        @if( (Auth::guard('admin')->user()->is_admin) == 2)
                            <li style="list-style: none;font-size: 16px;line-height: 40px;overflow: hidden;">
                                Chức vụ <span style="font-weight: 700;float: right;">Nhân viên bán hàng</span></li>
                        @endif
                ...
                <div class="min-w-0 p-4 bg-white rounded-lg shadow-xs dark:bg-gray-800">
                    ...
                    <form action="{{ route('profile.update',['id' => $admin->id]) }} }}" method="POST" enctype="multipart/form-data">
                        ...
                        <span class="text-gray-700 dark:text-gray-400">
                            ...
                            <input name="phone" value="{{$admin->phone}}" type="text" class="block w-full mt-1 text-sm dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray form-input" placeholder="Thêm tên người dùng " >
                            @error('phone')
                            <span class="text-xs text-red-600" role="alert">
                        <strong>{{ $message }}</strong>
                        </span>
        ...
        <form action="{{ route('profile.update-password-admin', ['id' => $admin->id]) }}" method="POST">
            @csrf
            @method('PUT')
